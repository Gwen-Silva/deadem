...
function diffSchema(expected, actual, prefix = '') {
    const diffs = [];
    if (typeof expected !== typeof actual || (typeof expected !== 'object' && expected !== actual)) {
        diffs.push({ field: prefix || '<root>', differenceType: 'schema_break', expected, actual });
        return diffs;
    }
    if (!expected || !actual || typeof expected !== 'object') return diffs;
    if (expected.type !== actual.type) {
        diffs.push({ field: prefix || '<root>', differenceType: 'schema_break', expected: expected.type, actual: actual.type });
        return diffs;
    }
    ...
    for (const key of [...keys].sort()) {
        ...
        if (!(key in (expected.fields ?? {}))) diffs.push({ field, differenceType: 'additional_field', expected: null, actual: actual.fields[key] });
        else if (!(key in (actual.fields ?? {}))) diffs.push({ field, differenceType: 'missing_field', expected: expected.fields[key], actual: null });
        else diffs.push(...diffSchema(expected.fields[key], actual.fields[key], field));
    }
    return diffs;
}
...
export async function buildCanonicalState(manifest, io, { clean = false } = {}) {
    ...
    const packageData = { playerRegistry, entityRegistry, factualEvents: events, nonTimelineMetadata, independentValidationOverlay, snapshots, capabilityMatrix };
    ...
    const provenanceAudit = {
        schemaVersion: 1,
        recordsChecked: events.length + snapshots.length + entities.length + players.length,
        missingSourcePath: events.filter(event => !event.provenance.sourcePath).length,
        missingEpistemicType: events.filter(event => !event.provenance.epistemicType).length,
        deterministicDerivationsWithoutMethod: events.filter(event => event.provenance.epistemicType === 'deterministic_derivation' && !event.provenance.method).length,
        directObservationMisusedForTeamNetWorth: events.filter(event => event.eventCategory === 'team_net_worth' && event.provenance.epistemicType === 'direct_parser_observation').length,
    ...
    const schemaDiff = {
        ...
        replay002VsReplay009KnownDifferences: [
            { category: 'independent_validation_overlay', status: 'source_unavailable_for_target_replay' },
            { category: 'entity_identity', status: 'v2_neutral_keys_replace_legacy_spatial_objective_ids' },
            { category: 'entity_generation', status: 'v2_marks_generation_unavailable_instead_of_fabricating' }
        ],
        schemaBreaks: []
    };
    ...
    const canonicalGate = {
        ...
        replayId: manifest.replayId,
        gate: manifest.expectedGate,
        readyWithConstraints: schemaValidation.valid && spatialFindings.length === 0 && identityAudit.fabricatedGenerationCount === 0
    };
    ...
    const correctionGate = {
        ...
        replayId: manifest.replayId,
        gate: manifest.expectedGate,
        success: schemaValidation.valid && spatialFindings.length === 0 && identityAudit.fabricatedGenerationCount === 0 && provenanceAudit.mechanicEffectsApplied === 0
    };
