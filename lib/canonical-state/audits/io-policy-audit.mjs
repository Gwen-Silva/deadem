...
import { assertPathWithinRoots, sha256File } from './common.mjs';
...
function modulePolicy(modulePath, manifest) {
    ...
    if (/finalize-replay-002-canonical-v[78]/u.test(modulePath) || /verify-replay-002-canonical-v[78]-(?:attestation|release-envelope)/u.test(modulePath)) {
...
function dynamicPathAllowed({ line, moduleText, policy }) {
    if (line.includes('samples/') || line.includes('output/replays/')) {
        return { allowed: false, classification: 'dynamic_path_may_resolve_forbidden_root', guard: null };
    }
    if (policy.role === 'factual_io_layer' && (moduleText.includes('assertAllowed') || moduleText.includes('assertGeneratedPath'))) {
        return { allowed: true, classification: 'dynamic_path_guarded_by_io_allowlist', guard: 'assertAllowed/assertGeneratedPath' };
    }
    if (line.includes('assertPathWithinRoots')) {
        return { allowed: true, classification: 'dynamic_path_guarded_by_assertPathWithinRoots', guard: 'assertPathWithinRoots' };
    }
    return { allowed: false, classification: 'unresolved_dynamic_path', guard: null };
}

function pathAllowed(pathLiteral, roots, context) {
    if (!pathLiteral) return dynamicPathAllowed(context);
...
function detectGuardedVariables(lines) {
    const guarded = new Map();
    for (const [index, line] of lines.entries()) {
        const assign = line.match(/\b(?:const|let|var)\s+(\w+)\s*=\s*assertPathWithinRoots\s*\(/u);
        if (assign) guarded.set(assign[1], { guard: 'assertPathWithinRoots', line: index + 1 });
        const reassignment = line.match(/^\s*(\w+)\s*=\s*assertPathWithinRoots\s*\(/u);
        if (reassignment) guarded.set(reassignment[1], { guard: 'assertPathWithinRoots', line: index + 1 });
    }
    return guarded;
}
...
export async function auditIoPolicy(manifest) {
    ...
    for (const modulePath of modules) {
        ...
        const guardedVariables = detectGuardedVariables(lines);
        lines.forEach((line, index) => {
            ...
            for (const api of FS_APIS) {
                ...
                let pathInfo = pathAllowed(literalPath(line), api.startsWith('write') || ['rm', 'mkdir'].includes(api) ? policy.allowedWriteRoots : policy.allowedReadRoots, { line, moduleText: text, policy });
                const arg = dynamicArgument(line, api, aliasEntry?.[0] ?? null);
                const variable = arg?.match(/^\w+$/u)?.[0] ?? null;
                const variableGuard = variable ? guardedVariables.get(variable) : null;
                if (!literalPath(line) && variableGuard) {
                    pathInfo = { allowed: true, classification: 'dynamic_path_guarded_by_tracked_variable', guard: variableGuard.guard, variable, variableOriginLine: variableGuard.line };
                }
                ...
                findings.push({
                    ...
                    expression: arg,
                    variable: pathInfo.variable ?? variable,
                    variableOriginLine: pathInfo.variableOriginLine ?? null,
