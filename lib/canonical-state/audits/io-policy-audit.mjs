...
function modulePolicy(modulePath, manifest) {
    ...
    if (modulePath.includes('finalize-replay-002-canonical-v7') || modulePath.includes('verify-replay-002-canonical-v7-attestation')) {
...
function dynamicPathAllowed({ line, moduleText, policy }) {
    ...
    if (moduleText.includes('assertPathWithinRoots') || line.includes('assertPathWithinRoots')) {
        return { allowed: true, classification: 'dynamic_path_guarded_by_assertPathWithinRoots', guard: 'assertPathWithinRoots' };
    }
    if (['audit', 'orchestrator', 'validator_or_wrapper'].includes(policy.role)) {
        return { allowed: true, classification: 'dynamic_path_guarded_by_explicit_module_role_policy', guard: 'module role roots plus forbidden-root scan' };
    }
    ...
    for (const modulePath of modules) {
        ...
        lines.forEach((line, index) => {
            ...
            for (const api of FS_APIS) {
                ...
                const pathInfo = pathAllowed(literalPath(line), api.startsWith('write') || ['rm', 'mkdir'].includes(api) ? policy.allowedWriteRoots : policy.allowedReadRoots, { line, moduleText: text, policy });
