...
export async function auditDocumentation(options = {}) {
    const expectedGate = options.expectedGate ?? 'replay_002_canonical_factual_state_ready_with_constraints_v8';
    const nextTaskPath = options.nextTaskPath ?? 'tasks/blocked/090-select-next-canonical-generalization-control.md';
    const reportPath = options.reportPath ?? 'reports/replay-002-canonical-factual-state-v8-validation.md';
    const task089 = await firstExistingPath([
        'tasks/active/089-make-release-decision-fail-closed-and-enforce-dynamic-io-guards.md',
        'tasks/completed/089-make-release-decision-fail-closed-and-enforce-dynamic-io-guards.md'
    ]);
    const files = [...BASE_FILES, task089 ?? 'tasks/active/089-make-release-decision-fail-closed-and-enforce-dynamic-io-guards.md', nextTaskPath];
    const corpus = {};
    const filesExamined = [];
    ...
    const rules = [
        fileRule('readme_declares_v8', ['README.md'], text => text.includes(expectedGate)),
        fileRule('project_state_declares_v8', ['docs/PROJECT_STATE.md'], text => text.includes(expectedGate)),
        fileRule('next_milestone_points_to_090', ['docs/NEXT_MILESTONE.md'], text => text.includes(nextTaskPath) || text.includes('Task 090')),
        fileRule('latest_points_to_v8_report', ['reports/latest.md'], text => text.includes(reportPath.replace('reports/', ''))),
        fileRule('index_points_to_v8_report', ['reports/INDEX.md'], text => text.includes(reportPath)),
        ...
        fileRule('task089_records_scope', [task089 ?? 'tasks/active/089-make-release-decision-fail-closed-and-enforce-dynamic-io-guards.md'], text => /Do not process replays 001|processar replays 001/u.test(text) && /Do not read, open, hash, or process replay 005|processar replay 005/u.test(text)),
        ...
        multiFileRule('no_stale_next_task_089_select', ['README.md', 'docs/PROJECT_STATE.md', 'docs/NEXT_MILESTONE.md', 'reports/latest.md'], text => !/next (task|follow-up)[^\n]*(089-select-next-canonical-generalization-control)/i.test(text))
    ];
