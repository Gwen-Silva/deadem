...
export async function auditDocumentation(options = {}) {
    const expectedGate = options.expectedGate ?? 'replay_002_canonical_factual_state_ready_with_constraints_v6';
    const nextTaskPath = options.nextTaskPath ?? 'tasks/blocked/088-select-next-canonical-generalization-control.md';
    const reportPath = options.reportPath ?? 'reports/replay-002-canonical-factual-state-v6-validation.md';
    const task087 = await firstExistingPath([
        'tasks/active/087-finalize-canonical-attestation-and-full-pipeline-determinism.md',
        'tasks/completed/087-finalize-canonical-attestation-and-full-pipeline-determinism.md'
    ]);
    const files = [...BASE_FILES, task087 ?? 'tasks/active/087-finalize-canonical-attestation-and-full-pipeline-determinism.md', nextTaskPath];
    const corpus = {};
    const filesExamined = [];
    ...
    const rules = [
        fileRule('readme_declares_v6', ['README.md'], text => text.includes(expectedGate)),
        fileRule('project_state_declares_v6', ['docs/PROJECT_STATE.md'], text => text.includes(expectedGate)),
        fileRule('next_milestone_points_to_088', ['docs/NEXT_MILESTONE.md'], text => text.includes(nextTaskPath) || text.includes('Task 088')),
        fileRule('latest_points_to_v6_report', ['reports/latest.md'], text => text.includes(reportPath.replace('reports/', ''))),
        fileRule('index_points_to_v6_report', ['reports/INDEX.md'], text => text.includes(reportPath)),
        ...
        fileRule('task087_records_scope', [task087 ?? 'tasks/active/087-finalize-canonical-attestation-and-full-pipeline-determinism.md'], text => text.includes('Do not process replays 001') && text.includes('Do not read, open, hash, or process replay 005')),
        ...
        multiFileRule('no_stale_next_task_087_select', ['README.md', 'docs/PROJECT_STATE.md', 'docs/NEXT_MILESTONE.md', 'reports/latest.md'], text => !/next (task|follow-up)[^\n]*(087-select-next-canonical-generalization-control)/i.test(text))
    ];
